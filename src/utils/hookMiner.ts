/**
 * Uniswap v4-style hook address miner (CREATE2) — browser + Hardhat.
...
/** True when compiled ABI looks like Continuity CounterHook. */
export function abiLooksLikeCounterHook(abi: unknown): boolean {
  ...
  return (
    names.has('sandboxBumpAfterSwap') &&
    names.has('requiredHookFlags') &&
    names.has('beforeSwap') &&
    names.has('afterSwap')
  );
}
